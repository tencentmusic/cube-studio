...
def partition_dataset(rank, datapath):
    """ Partitioning MNIST """
...
def run(modelpath, gpu, datapath):
    """ Distributed Synchronous SGD Example """
    rank = dist.get_rank()
    torch.manual_seed(1234)
    train_set, bsz = partition_dataset(rank, datapath)
    model = Net()
    if gpu:
        model = model.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datapath', metavar='path', required=True,
                        help='Path to model, e.g., /mnt/kubeflow-gcfs/pytorch/model')
    ...
    run(modelpath=args.modelpath, gpu=args.gpu,datapath = args.datapath)
