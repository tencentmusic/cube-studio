...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_chinese-base-ecom')
result = ner_pipeline('欧美单 秋季女装时尚百搭休闲修身 亚麻混纺短款 外套西装')

print(result)
# {'output': [{'type': 'HC', 'start': 6, 'end': 8, 'span': '女装'}, {'type': 'HC', 'start': 24, 'end': 26, 'span': '外套'}, {'type': 'HC', 'start': 26, 'end': 28, 'span': '西装'}]}
```
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_chinese-base-ecom
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于ecom-cn数据集上训练，在垂类领域中文文本上的NER效果会有降低，请用户自行评测后决定如何使用。
...
| --- | --- | --- | --- |
| ecom-cn | 80.65 | 82.1 | 81.36 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| CX | 69.47 | 67.44 | 68.44 |
| DM | 88.88 | 92.8 | 90.8 |
| GG | 78.96 | 84.05 | 81.42 |
| HC | 84.48 | 86.08 | 85.27 |
| HP | 79.68 | 80.26 | 79.97 |
| PX | 57.39 | 60.69 | 58.99 |
| RM | 81.46 | 82.82 | 82.13 |
| WHZP | 67.72 | 67.34 | 67.53 |
| XH | 73.89 | 79.11 | 76.41 |
