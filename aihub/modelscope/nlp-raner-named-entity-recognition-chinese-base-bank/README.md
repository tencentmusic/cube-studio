...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_chinese-base-bank')
result = ner_pipeline('请问中信银行信用卡业务怎么办理')
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_chinese-base-bank
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于bank数据集上训练，在垂类领域中文文本上的NER效果会有降低，请用户自行评测后决定如何使用。

## 训练数据介绍
- [bank](https://github.com/liucongg/NLPDataSet)银行借贷业务领域中文命名实体识别公开数据集，包括4种实体类型，共1500个句子。

| 实体类型 | 英文名 |
|----------|--------|
| 银行名 | BANK |
| 客户提到的形容词 | COMMENTS_ADJ |
| 客户提到的名词 | COMMENTS_N |
| 银行金融产品名 | PRODUCT |
...
| --- | --- | --- | --- |
| bank | 84.13 | 87.75 | 85.9 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| BANK | 94.28 | 96.92 | 95.58 |
| COMMENTS_ADJ | 71.69 | 78.28 | 74.84 |
| COMMENTS_N | 94.98 | 92.35 | 93.65 |
| PRODUCT | 85.03 | 91.91 | 88.34 |
