...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_chinese-base-finance')
result = ner_pipeline('刷单被骗1473元,花呗付款,可以追回吗')
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_chinese-base-finance
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于CCKS2021金融领域篇章级案件元素抽取数据集上训练，在垂类领域中文文本上的NER效果会有降低，请用户自行评测后决定如何使用。

## 训练数据介绍
- [Finance](https://www.biendata.xyz/competition/ccks_2021_task6_1) 金融领域中文案件元素抽取公开数据集，包括受害人、受害者身份、嫌疑人、支付渠道、案发城市、案发时间、涉案平台、资损金额、银行卡号等，共3865个句子。由于官方没有放出测试集，因此该模型是在dev集上f1值最高的。
...
| --- | --- | --- | --- |
| finance | 78.9 | 83.92 | 81.34 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| 受害人 | 86.96 | 90.19 | 88.54 |
| 受害人身份 | 38.46 | 41.67 | 40.0 |
| 嫌疑人 | 84.48 | 89.27 | 86.81 |
| 支付渠道 | 81.13 | 92.39 | 86.39 |
| 案发城市 | 59.35 | 62.66 | 60.96 |
| 案发时间 | 68.28 | 79.84 | 73.61 |
| 涉案平台 | 69.33 | 61.54 | 65.2 |
| 资损金额 | 84.66 | 86.65 | 85.64 |
| 银行卡号 | 50.0 | 100.0 | 66.67 |
