...
class CV_HRNETW48_HUMAN_WHOLEBODY_KEYPOINT_IMAGE_Model(Model):
    ...
    def inference(self, arg0, **kwargs):
        ...
        radius = max(1,height//100)
        # radius = 1
        for human in result['keypoints'].tolist():
            for index,keypoint in enumerate(human):
                if index<24:  # 包含68个人脸关键点、42个手势关键点、17个骨骼关键点和6个脚部关键点。
                    cv2.circle(img,(int(keypoint[0]),int(keypoint[1])),radius,(0, 0, 255), -1)
                else:
                    cv2.circle(img,(int(keypoint[0]),int(keypoint[1])),1,(0, 0, 255), -1)
