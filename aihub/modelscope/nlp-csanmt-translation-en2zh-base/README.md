...
- 2022年11月：
  - 发布英中-base模型，支持推理和垂直领域微调，请使用1.0.1及以上版本。相比[英中-big](https://www.modelscope.cn/models/damo/nlp_csanmt_translation_en2zh/summary)，本模型在新闻和口语两个场景上分别微降0.7和0.5BLEU，参数量降低70%，解码速度提升1.5倍。
...
pipeline_ins = pipeline(task=Tasks.translation, model="damo/nlp_csanmt_translation_en2zh_base")
...
trainer = CsanmtTranslationTrainer(model="damo/nlp_csanmt_translation_en2zh_base")
trainer.train()
