...
- UniASR印尼语语音识别流式模型，可对近场、低噪、正常语速、朗读形式的印尼语音频进行语音识别：
  - ASR模型：[UniASR模型](https://modelscope.cn/models/damo/speech_UniASR_asr_2pass-id-16k-common-vocab1067-tensorflow1-online/summary)，印尼语语音识别模型。
  - ITN模型：[印尼语ITN模型](https://modelscope.cn/models/damo/speech_inverse_text_processing_fun-text-processing-itn-id/summary)，可用于语音识别结果进行拟文本正则化后处理。
...
- 推理支持音频格式如下：
  ...
  - wav文件url，例如：https://isv-data.oss-cn-hangzhou.aliyuncs.com/ics/MaaS/ASR/test_audio/asr_example_id.wav
  - wav二进制数据，格式bytes，例如：用户直接从文件里读出bytes数据或者是麦克风录出bytes数据。
  - 已解析的audio音频，例如：audio, rate = soundfile.read("asr_example_id.wav")，类型为numpy.ndarray或者torch.Tensor。
  - wav.scp文件，需符合如下要求：
...
rec_result = inference_pipeline(audio_in='https://isv-data.oss-cn-hangzhou.aliyuncs.com/ics/MaaS/ASR/test_audio/asr_example_id.wav')
print(rec_result)
```

- 切换fast、normal、offline解码模式(默认normal模式)
```python
rec_result = inference_pipeline(audio_in='https://isv-data.oss-cn-hangzhou.aliyuncs.com/ics/MaaS/ASR/test_audio/asr_example_id.wav', param_dict={"decoding_model": "normal"})
```
...
inference_pipeline = pipeline(
    task=Tasks.auto_speech_recognition,
    model='damo/speech_UniASR_asr_2pass-id-16k-common-vocab1067-tensorflow1-offline',
...
waveform, sample_rate = torchaudio.load("asr_example_id.wav")
...
以[印尼语语音识别Common Voice数据集](https://modelscope.cn/datasets/modelscope/speech_asr_commonvoice_id_trainsets/summary)数据集为例，完整数据集已经上传ModelScope，可通过数据集英文名（speech_asr_commonvoice_id_trainsets）搜索：
...
ds_dict = MsDataset.load(dataset_name='speech_asr_commonvoice_id_trainsets', namespace='modelscope')
...
common_voice_id_25361422  halo dunia
common_voice_id_25377216  sudah makan sudah sholat

cat ./example_data/wav.scp
common_voice_id_25361422 /mnt/data/train/common_voice_id_25361422.wav
common_voice_id_25377216 /mnt/data/train/common_voice_id_25377216.wav
```
...
if __name__ == '__main__':
    ...
    params["model"] = "damo/speech_UniASR_asr_2pass-id-16k-common-vocab1067-tensorflow1-offline"
...
```sh
cd egs_modelscope/asr/uniasr/speech_UniASR_asr_2pass-id-16k-common-vocab1067-tensorflow1-online
python infer.py
```
