...
## 模型描述
该模型是基于检索增强(RaNer)方法在荷兰语数据集MultiCoNER-NL-Dutch训练的模型。
...
## 期望模型使用方式以及适用范围
本模型主要用于给输入荷兰语句子产出命名实体识别结果。用户可以自行尝试输入荷兰语句子。具体调用方式请参考代码示例。
...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_dutch-large-generic')
result = ner_pipeline('in het vorige seizoen promoveerden sc cambuur , dat kampioen werd en go ahead eagles via de play offs .')

print(result)
# {'output': [{'type': 'GRP', 'start': 35, 'end': 45, 'span': 'sc cambuur'}, {'type': 'GRP', 'start': 69, 'end': 84, 'span': 'go ahead eagles'}]}
```
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_dutch-large-generic
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于 MultiCoNER-NL-Dutch数据集上训练，在垂类领域荷兰语文本上的NER效果会有降低，请用户自行评测后决定如何使用。

## 训练数据介绍
- [MultiCoNER-NL-Dutch](https://www.amazon.science/publications/semeval-2022-task-11-multilingual-complex-named-entity-recognition-multiconer) 通用领域荷兰语命名实体识别公开数据集，包括地名, 人名, 公司名, 创作名, 消费品, 其他组织名，共235037个句子。
...
## 数据评估及结果
模型在 MultiCoNER-NL-Dutch验证数据评估结果:

| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
|  MultiCoNER-NL-Dutch | 97.32 | 97.23 | 97.28 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| CORP | 97.47 | 94.48 | 95.95 |
| CW | 98.88 | 96.7 | 97.78 |
| GRP | 93.6 | 98.77 | 96.12 |
| LOC | 97.67 | 98.33 | 98.0 |
| PER | 98.57 | 97.64 | 98.1 |
| PROD | 97.08 | 96.38 | 96.73 |
