...
class CV_CSPNET_IMAGE_OBJECT_DETECTION_YOLOX_NANO_COCO_Model(Model):
    ...
    hot = "3945"
    frameworks = "pytorch"
    doc = "https://modelscope.cn/models/damo/cv_cspnet_image-object-detection_yolox_nano_coco/summary"
    ...
    web_examples=[
        {
            "label": "示例1",
            "input": {
                "image": "/mnt/workspace/.cache/modelscope/damo/cv_cspnet_image-object-detection_yolox_nano_coco/res/test0.jpg"
            }
        },
        {
            "label": "示例2",
            "input": {
                "image": "/mnt/workspace/.cache/modelscope/damo/cv_cspnet_image-object-detection_yolox_nano_coco/res/test1.jpg"
            }
        }
    ]
    ...
    def load_model(self,save_model_dir=None,**kwargs):
        ...
        self.p = pipeline('image-object-detection', 'damo/cv_cspnet_image-object-detection_yolox_nano_coco')
...
model=CV_CSPNET_IMAGE_OBJECT_DETECTION_YOLOX_NANO_COCO_Model()

# 测试后将此部分注释
# model.load_model()
# result = model.inference(image='/mnt/workspace/.cache/modelscope/damo/cv_cspnet_image-object-detection_yolox_nano_coco/res/test0.jpg')  # 测试
# print(result)
...
Tested by: 秋水泡茶
模型大小：18M
模型效果：支持80类通用目标检测，对图片中明显的大目标具有良好的检测效果，但对中等目标、小目标和遮挡目标存在漏检情况，泛化性能不及yolox原模型。
推理性能：以1330*1330的图片为例，推理速度平均为400ms。以720*1280的图片为例，推理速度最快280ms，最慢为490ms。
测试环境：CPU：Intel(R) Xeon(R) Gold 5118 CPU @ 2.30GHz, GPU：Tesla V100 32G
占用GPU显存：启动时内存占用约为1453M，推理时占用约为1523M。
巧妙使用方法：
'''