...
## 期望模型使用方式以及适用范围
本模型主要用于给输入德语句子产出命名实体识别结果。用户可以自行尝试输入德语句子。具体调用方式请参考代码示例。
...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_german-large-generic')
result = ner_pipeline('die szene beinhaltete lenny baker und christopher walken .')

print(result)
# {'output': [{'type': 'PER', 'start': 22, 'end': 33, 'span': 'lenny baker'}, {'type': 'PER', 'start': 38, 'end': 56, 'span': 'christopher walken'}]}
```
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_german-large-generic
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于MultiCoNER-DE-German数据集上训练，在垂类领域德语文本上的NER效果会有降低，请用户自行评测后决定如何使用。

## 训练数据介绍
- [MultiCoNER-DE-German](https://www.amazon.science/publications/semeval-2022-task-11-multilingual-complex-named-entity-recognition-multiconer) 通用领域德语命名实体识别公开数据集，包括地名, 人名, 公司名, 创作名, 消费品, 其他组织名，共235524个句子。
...
## 数据评估及结果
模型在MultiCoNER-DE-German验证数据评估结果:

| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| MultiCoNER-DE-German | 94.85 | 95.16 | 95.0 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| CORP | 93.21 | 91.52 | 92.35 |
| CW | 93.19 | 94.18 | 93.68 |
| GRP | 92.36 | 90.62 | 91.48 |
| LOC | 97.29 | 96.96 | 97.12 |
| PER | 97.34 | 98.99 | 98.16 |
| PROD | 91.24 | 93.98 | 92.59 |
