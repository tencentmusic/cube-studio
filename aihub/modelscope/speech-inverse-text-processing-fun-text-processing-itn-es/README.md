...
西班牙语逆文本正则化模型是基于[FunTextProcessing](https://github.com/alibaba-damo-academy/FunASR/tree/main/fun_text_processing)开源代码库生成，用于西班牙语语音识别模型结果后处理中的逆文本正则化部分。
...
itn_inference_pipline = pipeline(
    ...
    model='damo/speech_inverse_text_processing_fun-text-processing-itn-es')

itn_result = itn_inference_pipline(text_in='ciento veintitrés')
print(itn_result)
```
