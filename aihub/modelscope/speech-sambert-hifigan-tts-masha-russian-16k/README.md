...
text = 'Воспоминаний масса, и они останутся со мной.'
model_id = 'speech_tts/speech_sambert-hifigan_tts_masha_Russian_16k'
...
## 训练数据介绍
使用6个不同发音人，共计约130小时俄文语料数据训练。

## 模型训练流程
模型所需训练数据格式为：音频(.wav), 文本(.txt),  随机初始化训练要求训练数据规模在2小时以上，对于2小时以下的数据集，需使用多人预训练模型进行参数初始化。其中，AM模型训练时间需要1～2天，Vocoder模型训练时间需要5～7天。

### 预处理
模型训练需对音频文件提取声学特征(梅尔频谱)；文本字符转换成对应字节的one-hot编号；
