...
德语逆文本正则化模型是基于[FunTextProcessing](https://github.com/alibaba-damo-academy/FunASR/tree/main/fun_text_processing)开源代码库生成，用于德语语音识别模型结果后处理中的逆文本正则化部分。
...
Verbalizer则是将parse和调序之后的tokens调用相关类型的Verbalizer输出，获得最后的结果。同时基于[FunASR/fun_text_processing](https://github.com/alibaba-damo-academy/FunASR/tree/main/fun_text_processing)，开源了设计和生成这些ITN规则的工具。该工具提供了安装、测试、导出的python工具包。具体的使用方法可以参考FunASR/fun_text_processing中的[README.md](https://github.com/alibaba-damo-academy/FunASR/blob/main/fun_text_processing/README.md)。
...
- 修改：利用FunASR/fun_text_processing/inverse_text_normalization相应语言的不同语法规则进行修改。
...
itn_inference_pipline = pipeline(
    ...
    model='damo/speech_inverse_text_processing_fun-text-processing-itn-de')

itn_result = itn_inference_pipline(text_in='einhundertdreiundzwanzig')
print(itn_result)
```
