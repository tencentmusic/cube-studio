...
## 模型描述
该模型是基于检索增强(RaNer)方法在土耳其语数据集MultiCoNER-TR-Turkish训练的模型。
...
## 期望模型使用方式以及适用范围
本模型主要用于给输入土耳其语句子产出命名实体识别结果。用户可以自行尝试输入土耳其语句子。具体调用方式请参考代码示例。
...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_turkish-large-generic')
result = ner_pipeline('köyün pırasa kavurması ve içi yağlama ve akıtma adındaki hamur işleri meşhurdur . ; çörek ekmeği ; diye adlandırdıkları mayasız ekmeği unutmamaklazım .')

print(result)
# {'output': [{'type': 'PROD', 'start': 6, 'end': 12, 'span': 'pırasa'}]}
```
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_turkish-large-generic
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于MultiCoNER-TR-Turkish数据集上训练，在垂类领域土耳其语文本上的NER效果会有降低，请用户自行评测后决定如何使用。

## 训练数据介绍
- [MultiCoNER-TR-Turkish](https://www.amazon.science/publications/semeval-2022-task-11-multilingual-complex-named-entity-recognition-multiconer) 通用领域土耳其语命名实体识别公开数据集，包括地名, 人名, 公司名, 创作名, 消费品, 其他组织名，共235518个句子。
...
## 数据评估及结果
模型在MultiCoNER-TR-Turkish验证数据评估结果:

| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| MultiCoNER-TR-Turkish | 97.91 | 97.75 | 97.83 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| CORP | 96.6 | 95.95 | 96.27 |
| CW | 95.83 | 96.84 | 96.34 |
| GRP | 97.56 | 95.81 | 96.68 |
| LOC | 99.14 | 98.58 | 98.86 |
| PER | 97.85 | 98.7 | 98.28 |
| PROD | 99.37 | 99.37 | 99.37 |
