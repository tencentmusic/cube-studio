...
- UniASR韩语语音识别模型，可对近场、低噪、正常语速、朗读形式的韩语音频进行语音识别：
  - ASR模型：[UniASR模型](https://modelscope.cn/models/damo/speech_UniASR_asr_2pass-id-16k-common-vocab6400-tensorflow1-offline/summary)，韩语语音识别模型。
  - ITN模型：[韩语ITN模型](https://modelscope.cn/models/damo/speech_inverse_text_processing_fun-text-processing-itn-ko/summary)，可用于语音识别结果进行拟文本正则化后处理。
...
- 推理支持音频格式如下：
  ...
  - wav文件url，例如：https://isv-data.oss-cn-hangzhou.aliyuncs.com/ics/MaaS/ASR/test_audio/asr_example_ko.wav
  - wav二进制数据，格式bytes，例如：用户直接从文件里读出bytes数据或者是麦克风录出bytes数据。
  - 已解析的audio音频，例如：audio, rate = soundfile.read("asr_example_ko.wav")，类型为numpy.ndarray或者torch.Tensor。
  - wav.scp文件，需符合如下要求：
...
rec_result = inference_pipeline(audio_in='https://isv-data.oss-cn-hangzhou.aliyuncs.com/ics/MaaS/ASR/test_audio/asr_example_ko.wav')
print(rec_result)
```
...
waveform, sample_rate = torchaudio.load("asr_example_ko.wav")
...
以[韩语语音识别Common Voice数据集](https://modelscope.cn/datasets/modelscope/speech_asr_commonvoice_ko_trainsets/summary)数据集为例，完整数据集已经上传ModelScope，可通过数据集英文名（speech_asr_commonvoice_ko_trainsets）搜索：
...
ds_dict = MsDataset.load(dataset_name='speech_asr_commonvoice_ko_trainsets', namespace='modelscope')
...
common_voice_ko_35844026        골짜구니가 울리도록 커다랗게 기침을 하였다
common_voice_ko_35844025        백 모의 아들인가

cat ./example_data/wav.scp
common_voice_ko_35844026 /mnt/data/train/common_voice_ko_35844026.wav
common_voice_ko_35844025 /mnt/data/train/common_voice_ko_35844025.wav
```
