...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_english-large-social_media')
result = ner_pipeline('Becky in a Snickers advert ?')

print(result)
# {'output': [{'type': 'person', 'start': 0, 'end': 5, 'span': 'Becky'}, {'type': 'corporation', 'start': 11, 'end': 19, 'span': 'Snickers'}]}
```
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_english-large-social_media
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于社交媒体数据集上训练，在垂类领域英文文本上的NER效果会有降低，请用户自行评测后决定如何使用。

## 训练数据介绍
- [wnut17](http://noisy-text.github.io/2017/files/) 社交媒体领域英语命名实体识别公开数据集，包括corporation, creative-work, group, location, person, product六个类别，共5690个句子。
...
| --- | --- | --- | --- |
| wnut17 | 69.41 | 52.36 | 59.69 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| corporation | 44.26 | 40.91 | 42.52 |
| creative-work | 68.42 | 36.62 | 47.71 |
| group | 61.18 | 31.52 | 41.6 |
| location | 74.42 | 64.0 | 68.82 |
| person | 75.78 | 73.66 | 74.7 |
| product | 47.83 | 17.32 | 25.43 |
