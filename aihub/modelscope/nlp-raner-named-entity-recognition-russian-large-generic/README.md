...
## 模型描述
该模型是基于检索增强(RaNer)方法在俄语数据集MultiCoNER-RU-Russian训练的模型。
...
ner_pipeline = pipeline(Tasks.named_entity_recognition, 'damo/nlp_raner_named-entity-recognition_russian-large-generic')
result = ner_pipeline('важным традиционным промыслом является производство пальмового масла .')

print(result)
# {'output': [{'type': 'PROD', 'start': 52, 'end': 68, 'span': 'пальмового масла'}]}
```
...
model:
  ...
  embedder:
    model_name_or_path: damo/nlp_raner_named-entity-recognition_russian-large-generic
  dropout: 0.1
  use_crf: true
...
### 模型局限性以及可能的偏差
本模型基于MultiCoNER-RU-Russian数据集上训练，在垂类领域俄语文本上的NER效果会有降低，请用户自行评测后决定如何使用。

## 训练数据介绍
- [MultiCoNER-RU-Russian](https://www.amazon.science/publications/semeval-2022-task-11-multilingual-complex-named-entity-recognition-multiconer) 通用领域俄语命名实体识别公开数据集，包括地名, 人名, 公司名, 创作名, 消费品, 其他组织名，共235201个句子。
...
## 数据评估及结果
模型在MultiCoNER-RU-Russian验证数据评估结果:

| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| MultiCoNER-RU-Russian | 94.51 | 95.78 | 95.14 |

各个类型的性能如下: 
| Dataset | Precision | Recall | F1 |
| --- | --- | --- | --- |
| CORP | 94.41 | 95.6 | 95.0 |
| CW | 95.83 | 95.83 | 95.83 |
| GRP | 94.67 | 94.04 | 94.35 |
| LOC | 90.95 | 95.48 | 93.16 |
| PER | 95.88 | 96.88 | 96.37 |
| PROD | 96.69 | 96.69 | 96.69 |
