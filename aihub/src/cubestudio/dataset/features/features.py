...
from .label.classlabel import ClassLabel
...
class Features(dict):
    ...
    def arrow_schema(self):
        ...
        Returns:
            :obj:`pyarrow.Schema`
        """
        # metadata信息用来从schema转变回feature类型
        feature_metadata = {"info": {"features": self.to_dict()}}
        return pa.schema(self.type).with_metadata({"cube-studio": json.dumps(feature_metadata)})
    ...
    def from_arrow_schema(cls, pa_schema: pa.Schema) -> "Features":
        ...
        if pa_schema.metadata is not None and "cube-studio".encode("utf-8") in pa_schema.metadata:
            metadata = json.loads(pa_schema.metadata["cube-studio".encode("utf-8")].decode())
