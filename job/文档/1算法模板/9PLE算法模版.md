PLE算法模板配置示例如下（可以先阅读[《pipeline各任务配置》文档](http://tapd.oa.com/kubeflow/markdown_wikis/show/#1220424693001722117)中的"tensorflow模型训练任务配置.runner封装方式"一节了解模型训练任务的一般配置方法）:  
...
{  
    ...
    "trainer": "runner",  
    "job_detail": {  
        "model_input_config_file": "${PACK_PATH}$/pr.json",  
        "model_args": {  
            "ordered_task_names": [  
                "ctr_label",  
                "cvr_label"  
        ...
        "train_data_args": {  
            "data_file": "${PACK_PATH}$/train1.csv",  
            "file_type": "CSV",  
            "batch_size": 1024,  
            "field_delim": " ",  
            "with_headers": false,  
        ...
        "val_data_args": {  
            ...
            "shuffle_buffer_size": 2048,  
            "shuffle": false,  
            "model_input_config_file": "${PACK_PATH}$/pr_gauc.json"  
        },  
        "train_args": {  
            "mw_com": "RING",  
...
**model_args**中是特定于PLE模型的参数：  
  
  
- **custom_layer_config_file**：非必填。线上的模版不需要填写，本机调试的时候需要补充，指定自定义网络层的文件路径。默认路径在job-template工程的job/model_template/tf/ple/custom/custom_layers.py  
- **ordered_task_name**：必填。PLE中的任务的顺序列表  
- **layer_number**：必填。表示PLE层的数量  
- **ple_dict**：必填。设置每个任务下的各个专家网络的class名称，专家网络放置在"custom_layer_config_file"里面。其中任务名称为"shared_experts"的任务是共享任务，固定字符串标识。另外每个任务下的字典的key是数字字符串，值从0开始递增的数字字符串，例如："0"，"1"，“2”。  
- **tower_dict**：必填。各个任务的tower层的class名称。  
- **is_concat_gate_input**：非必填。true则表示把上一层的全部输出concat起来作为每个task gate的输入，false则表示每个task把上一层该task的输出作为该task gate的输入。默认为true。  
- **use_inputs_dropout**：非必填。是否需要将input进行dropout。默认true。  
- **name**：非必填。模型名字，可以自定义。默认为"ple".  
  
  
**predict_args**配置参考[tensorflow模型离线预测任务配置](http://tapd.oa.com/kubeflow/markdown_wikis/show/#1220424693001722117@toc13)，**注意模板中script_name参数和predict_args中的model_path参数都不是必填的。**  
