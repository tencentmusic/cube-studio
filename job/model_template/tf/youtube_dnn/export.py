...
from job.model_template.tf.youtube_dnn.custom_objects import custom_objects
from job.pkgs.tf.feature_util import *
from job.pkgs.utils import even_spread_num
...
class ItemEmbedddingExportProcess(object):
    ...
    def __call__(self, model_path, predict_data_files, item_list_col_descs: List[InputDesc],
                 ...
                 write_buf_len=None, emb_type='ef', ts_bn=None, csv_column_names=None, csv_field_delim=' '):
        ...
        if item_list_col_descs is not None:
            ...
            if item_list_col_descs:
                ...
                for item_list_col_desc in item_list_col_descs:
                    ds = tf.data.experimental.make_csv_dataset(predict_data_files, column_names=csv_column_names,
                                                               batch_size=1024, num_epochs=1,
                                                               field_delim=csv_field_delim,
        ...
        if item_col_desc is not None:
            item_ds = tf.data.experimental.make_csv_dataset(predict_data_files, column_names=csv_column_names,
                                                            batch_size=1024, num_epochs=1, field_delim=csv_field_delim,
                                                            num_parallel_reads=tf.data.experimental.AUTOTUNE,
                                                            prefetch_buffer_size=tf.data.experimental.AUTOTUNE,
        ...
        with open(vec_file, 'w', buffering=2 ** 25) as f:
            for item_ids in all_item_ds:
                ...
                embeddings = model.predict_item_embeddings(tf.reshape(item_ids, (-1, 1)))
                ...
                for item_id, embedding in zip(item_ids, embeddings):
                    ...
                    if emb_type == 'ts':
                        line = '{' + \
                               '"MD": "{}","BN": "{}", "VA": "txt|{}"'.format(item_id, ts_bn, embedding_str)\
                               + '}'
                    elif emb_type == 'ef':
                        line = "{}|{}|{}".format(item_id, item_id, embedding_str)
                    else:
                        line = "{}\t{}".format(item_id, embedding_str)
                    f.write(line + '\n')
                    write_cnt += 1
...
class UserEmbeddingExportProcess(object):
    ...
    def __call__(self, model_path, predict_data_files, user_input_descs: List[InputDesc],
                 ...
                 write_buf_len=None, emb_type='ef', ts_bn=None):
        print("{}: started, predict_data_files={}, vec_file={}".format(self.name, predict_data_files, vec_file))
        if not predict_data_files:
            print("{}: predict_data_files not specifed: {}".format(self.name, predict_data_files))
            return 0
        ...
        with open(vec_file, 'w', buffering=2**25) as f:
            ...
            for batch in ds:
                uids = batch.pop(uid_col_desc.name)
                pred_st = time.perf_counter()
                ...
                for uid, embedding in zip(uids, embeddings):
                    ...
                    if emb_type == 'ts':
                        line = '{' + \
                               '"MD": "{}","BN": "{}", "VA": "txt|{}"'.format(uid, ts_bn, embedding_str)\
                               + '}'
                    elif emb_type == 'ef':
                        line = "{}|{}|{}".format(uid, uid, embedding_str)
                    else:
...
def export_item_embeddings(model_path, predict_data_files, item_list_col_descs: List[InputDesc],
                           item_col_desc: InputDesc, vec_file, predict_batch_size=4096,
                           write_buf_len=None, parallel=None, emb_type='ef', ts_bn=None,
                           csv_column_names=None, csv_field_delim=' '):
    ...
    for i, (srcs, tar) in enumerate(src_tar_pairs):
        ret = ppool.submit(ItemEmbedddingExportProcess(i), model_path, srcs, item_list_col_descs,
                           item_col_desc, tar, predict_batch_size, write_buf_len, emb_type, ts_bn,
                           csv_column_names, csv_field_delim)
        p_rets.append(ret)
...
def export_user_embeddings(model_path, predict_data_files, user_input_descs: List[InputDesc],
    ...
    for i, (srcs, tar) in enumerate(src_tar_pairs):
        ret = ppool.submit(UserEmbeddingExportProcess(i), model_path, srcs, user_input_descs,
                           uid_col_desc, tar, predict_batch_size, write_buf_len, emb_type, ts_bn)
        p_rets.append(ret)
