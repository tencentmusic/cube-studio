...
from job.model_template.tf.mind.export import export_user_embeddings, export_item_embeddings
from job.pkgs.utils import make_abs_or_data_path

DEBUG = False


class MINDModelTemplateDriver(ModelTemplateDriver):
    def __init__(self, output_file=None, args=None, local=False):
        super(MINDModelTemplateDriver, self).__init__("MIND model template" + ("(debuging)" if DEBUG else ""),
...
if __name__ == "__main__":
    if DEBUG:
        ...
        import argparse
        parser = argparse.ArgumentParser(description="mindmodel arguments")
        parser.add_argument('--use_gpu', help='if use gpu during training and evaluating', default=False, action='store_true')
        # parser.add_argument('--config_input_path', type=str, help='config input path', default='/data/ft_local/dataset_30songs/input_config_30songs.json')
        # parser.add_argument('--config_template_path', type=str, help='config template path', default='/data/ft_local/dataset_30songs/mind_template_config_30songs.json')
        # parser.add_argument('--export_path', type=str, help='export path', default='/data/ft_local/job-runs/runs-30songs1/')
        ...
        parser.add_argument('--config_template_path', type=str, help='config template path', default='/data/ft_local/dataset_movielens/mind_template_config_movielens.json')
        parser.add_argument('--export_path', type=str, help='export path', default='/data/ft_local/job-runs/runs-movielens-item-n/')
        # parser.add_argument('--config_input_path', type=str, help='config input path', default='/data/ft_local/dataset_playlist/playlist_input_config.json')
        # parser.add_argument('--config_template_path', type=str, help='config template path', default='/data/ft_local/dataset_playlist/mind_template_config_playlist.json')
        # parser.add_argument('--export_path', type=str, help='export path', default='/data/ft_local/job-runs/runs-playlist1/')
        ...
        with open(args.config_template_path, 'r') as jcf:
            ...
            driver = MINDModelTemplateDriver(args=[
        ...
        driver = MINDModelTemplateDriver()
        driver.run()
