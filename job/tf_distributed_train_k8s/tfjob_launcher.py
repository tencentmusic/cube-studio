...
def default_job_name():
    ...
    # return "-".join([str(ctx.creator), p_name, "tfjob", str(uuid.uuid1())])
    ...
    if GPU_TYPE=='TENCENT' and GPU_RESOURCE:
        if len(GPU_RESOURCE.split(','))==2:
            ...
            if gpu_core and gpu_mem:
                pod_spec['template']['spec']['containers'][0]['resources']['requests']['tencent.com/vcuda-core'] = gpu_core
                pod_spec['template']['spec']['containers'][0]['resources']['requests']['tencent.com/vcuda-memory'] = gpu_mem
                pod_spec['template']['spec']['containers'][0]['resources']['limits']['tencent.com/vcuda-core'] = gpu_core
                pod_spec['template']['spec']['containers'][0]['resources']['limits']['tencent.com/vcuda-memory'] = gpu_mem
...
def launch_tfjob(name, num_workers, image,working_dir, worker_command):
    ...
    由给定参数启动tfjob进行模型训练
    Args:
        name: TFJob任务名字，如果不传，默认为"tfjob_<uuid>"
        namespace: tfjob的namespace，如果不传，默认为"kubeflow"
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--image', type=str, help="运行job的镜像", default='csighub.tencentyun.com/tme-kubeflow/pytorch-dist-mnist-test:1.0')

    args = arg_parser.parse_args()
    print("{} args: {}".format(__file__, args))

    run_shell("echo '10.101.140.98 cls-g9v4gmm0.ccs.tencent-cloud.com' >> /etc/hosts")
