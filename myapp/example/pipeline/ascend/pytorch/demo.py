...
def main():
    ...
    if should_distribute():
        ...
        dist.init_process_group(backend="hccl", rank=int(os.getenv('RANK',0)), world_size=int(os.getenv('WORLD_SIZE',1)))
    ...
    if is_distributed():
        GPU_NUM = os.environ.get('GPU_NUM','1')
        GPU_NUM = int(GPU_NUM.split('(')[0])
        model = nn.parallel.DistributedDataParallel(model,device_ids=list(range(GPU_NUM)))
