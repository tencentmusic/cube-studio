...
def deliver_message(job):
    ...
    if job.status in info_json['alert_status'] and job.status not in info_json['has_push']:
        ...
        if workflow_name:
            ...
            message = "pytorchjob: %s \nhp: %s(%s) \nnamespace: %s\nstatus: %s \ntime: %s" % (job.name,info_json.get('hp_name',''),info_json.get('describe',''),job.namespace,job.status,datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        else:
...
#     task_monitorings = json.loads(task.monitoring).get('pytorchjob',[])
#     if len(task_monitorings)>9:
#         max_cpu = 0
#         max_memory=0
#         init_message = 'pipeline(%s)中分布式训练%s，推荐资源如下，自行修改:\n' % (task.pipeline.describe,task.label)
#         message = init_message
#         # pytorchjob_src_mem=re.match(task.args.match("memory": "32G",))
    ...
    try:
        if pytorchjob.status=='Succeeded':
            ...
            if task_id:
                ...
                # 清理监控记录
                pytorchjob_monitoring_new = []
                for metric in pytorchjob_monitoring:
                    ...
                    if float(metric.get('cpu', 0)) > 0.1 and float(metric.get('memory', 0)) > 0.1 and metric['update_time'] > (datetime.datetime.now() - datetime.timedelta(days=30)).strftime('%Y-%m-%d %H:%M:%S'):
                        pytorchjob_monitoring_new.append(metric)

                if len(pytorchjob_monitoring_new) > 10:
                    del pytorchjob_monitoring_new[0]

                monitoring_new = {}
                monitoring_new['task'] = monitoring.get('task', [])
                monitoring_new['pytorchjob'] = pytorchjob_monitoring_new
...
def save_history(pytorchjob,dbsession):
    ...
    if info_json['has_push']:
        if not pytorchjob.status in info_json['has_push']:
            info_json['has_push'] += ',' + pytorchjob.status
    else:
        info_json['has_push'] = pytorchjob.status
    pytorchjob.info_json = json.dumps(info_json, indent=4, ensure_ascii=False)
    dbsession.commit()
