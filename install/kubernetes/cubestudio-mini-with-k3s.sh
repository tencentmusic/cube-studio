...
check_k3s_installed() {
    ...
    # 检查是否能够获取到节点信息，作为安装成功的进一步验证
    if ! k3s kubectl get nodes > /dev/null 2>&1; then
        echo "K3s is installed but cannot retrieve node information."
        return 3
    fi

    echo "K3s is installed and running successfully."
    return 0
}
...
initialize_environment() {
    ...
    mkdir -p /data/k8s/kubeflow/pipeline/workspace \
             ...
             /data/k8s/infra/mysql \
             /data/k8s/kubeflow/minio/mlpipeline \
             /data/k8s/kubeflow/global \
             /data/k8s/monitoring/grafana/ \
             /data/k8s/monitoring/prometheus/ \
             /data/k8s/kubeflow/labelstudio/
    ...
    # 并行拉取容器镜像
    k3s crictl pull busybox:1.36.0 &
    k3s crictl pull nvidia/dcgm-exporter:3.1.7-3.1.4-ubuntu20.04 &
    k3s crictl pull grafana/grafana:9.1.5 &
    k3s crictl pull ccr.ccs.tencentyun.com/cube-studio/prometheus-adapter:v0.9.1 &
    k3s crictl pull quay.io/prometheus-operator/prometheus-operator:v0.46.0 &
    k3s crictl pull quay.io/prometheus-operator/prometheus-config-reloader:v0.46.0 &
    k3s crictl pull volcanosh/vc-scheduler:v1.7.0 &
    k3s crictl pull mysql:8.0.32 &
    k3s crictl pull volcanosh/vc-controller-manager:v1.7.0 &
    k3s crictl pull argoproj/workflow-controller:v3.4.3 &
    k3s crictl pull carlosedp/addon-resizer:v1.8.4 &
    k3s crictl pull istio/proxyv2:1.15.0 &
    k3s crictl pull minio/minio:RELEASE.2023-04-20T17-56-55Z &
    k3s crictl pull kubeflow/training-operator:v1-8a066f9 &
    k3s crictl pull prom/prometheus:v2.27.1 &
    k3s crictl pull argoproj/argocli:v3.4.3 &
    k3s crictl pull volcanosh/vc-webhook-manager:v1.7.0 &
    k3s crictl pull bitnami/redis:6.2.12 &
    k3s crictl pull argoproj/argoexec:v3.4.3 &
    k3s crictl pull bitnami/kube-rbac-proxy:0.14.1 &
    k3s crictl pull kubernetesui/metrics-scraper:v1.0.8 &
    k3s crictl pull nvidia/k8s-device-plugin:v0.11.0-ubuntu20.04 &
    k3s crictl pull prom/node-exporter:v1.5.0 &
    k3s crictl pull istio/pilot:1.15.0 &
    k3s crictl pull kubernetesui/dashboard:v2.6.1 &
    k3s crictl pull ccr.ccs.tencentyun.com/cube-studio/kubeflow-dashboard-frontend:2024.03.01
    k3s crictl pull ccr.ccs.tencentyun.com/cube-studio/kubeflow-dashboard:2024.03.01
    # 等待所有后台进程完成
    wait
    set -e
}

copy_kubeconfig_to_currentdir() {
    # 定义源文件和目标文件路径
    set +e
    echo "--------copy config to current dir--------------"
    local src_file="/etc/rancher/k3s/k3s.yaml"
...
install_cube_studio() {
    set +e
    echo "--------start install cube-studio--------------"
    # node=`kubectl  get node -o wide  | awk 'NR==2 {print $1}' | head -n 1`
    # nodeIp=`kubectl  get node -o wide  | awk 'NR==2 {print $6}' | head -n 1`
    node=$(kubectl get node -o wide | awk 'NR==2 {print $1}' | head -n 1)
    nodeIp=$(kubectl get node -o wide | awk 'NR==2 {print $6}' | head -n 1)
    ...
    kubectl delete -f redis/redis.yaml 2>/dev/null
    kubectl create -f redis/redis.yaml

    kubectl delete -f volcano/volcano-development.yaml 2>/dev/null
    ...
    # 部署管理平台
    kubectl delete configmap kubernetes-config -n infra 2>/dev/null
    kubectl create configmap kubernetes-config --from-file=kubeconfig -n infra

    kubectl delete configmap kubernetes-config -n pipeline 2>/dev/null
    kubectl create configmap kubernetes-config --from-file=kubeconfig -n pipeline

    kubectl delete configmap kubernetes-config -n automl 2>/dev/null
    ...
    kubectl delete -k cube/overlays 2>/dev/null 
    kubectl apply -k cube/overlays

    # 配置入口
    #ip=`ifconfig eth1 | grep 'inet '| awk '{print $2}' | head -n 1`
    kubectl patch svc istio-ingressgateway -n istio-system -p '{"spec":{"externalIPs":["'"$nodeIp"'"]}}'

    # 本地电脑手动host
    echo "获取所有pod状态命令: kubectl get po -A"
    echo "打开网址：http://$nodeIp"
    set -e
    return 0
}
