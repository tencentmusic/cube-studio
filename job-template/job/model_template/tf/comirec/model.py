...
from job.pkgs.tf.extend_layers import ModelInputLayer, CapsuleLayer, PositionalEncodingLayer, DNNLayer
...
class ComiRecModel(tf.keras.Model, ABC):
    def __init__(self, model_input_config: ModelInputConfig, 
        ...
        if interest_extractor.lower()=='dr':
            ...
            if add_pos:
                self.positional_encoding_layer = PositionalEncodingLayer(dim=item_embedding_dim, learnable=True)
