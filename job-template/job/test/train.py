...
def main(args):
    # 读取数据
    print("正在读取数据...")
    data = pd.read_csv(args.input_csv)  # 你的CSV文件
    ...
    model = DecisionTreeClassifier(
        max_depth=int(args.max_depth),           # 限制树的最大深度，防止过拟合
        min_samples_split=int(args.min_samples_split),  # 内部节点再划分所需最小样本数
        min_samples_leaf=int(args.min_samples_leaf),    # 叶节点最少样本数
        random_state=42
    )
    ...
    # 方法1: 使用joblib保存模型（推荐）
    print("\n使用joblib保存模型...")
    model_filename = os.path.join(args.output_dir,'decision_tree_model.pkl')
    joblib.dump(model, model_filename, compress=3)
    print(f"模型已保存到: {model_filename}")

    # 方法2: 同时保存为pickle格式（兼容性更好）
    print("\n使用pickle保存模型...")
    model_filename = os.path.join(args.output_dir,'decision_tree_model_pickle.pkl')
    ...
    data_stats_path = os.path.join(args.output_dir,'data_stats.json')
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--input_csv', type=str, help="输入文件地址", default='train.csv')
    arg_parser.add_argument('--max_depth', type=str, help="超参数1", default='5')
    arg_parser.add_argument('--min_samples_split', type=str, help="超参数2", default='10')
    arg_parser.add_argument('--min_samples_leaf', type=str, help="超参数3", default='5')
    arg_parser.add_argument('--output_dir', type=str, help="保存目录", default='output')
